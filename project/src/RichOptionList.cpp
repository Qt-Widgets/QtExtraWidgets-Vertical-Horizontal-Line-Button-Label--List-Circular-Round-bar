#include "RichOptionList.hpp"
#include "ThemeHandler.hpp"

RichOptionListItemDelegate::RichOptionListItemDelegate(bool large, QObject *parent)
{
...
void RichOptionListItemDelegate::paint(QPainter * painter, const QStyleOptionViewItem & option, const QModelIndex & index) const
{
	...
	QPen penItemSeparator(currentThemePalette().text(), 1, Qt::SolidLine);
	...
	QFont fontDescriptionBold(currentFont(), 8, QFont::Bold);
...
QSize RichOptionListItemDelegate::sizeHint ( const QStyleOptionViewItem & option, const QModelIndex & index ) const
{
    return QSize(200, rowHeight);
}

RichOptionListItemDelegate::~RichOptionListItemDelegate()
{

}

RichOptionList::RichOptionList(bool large)
{
    setItemDelegate(new RichOptionListItemDelegate(large, this));
}

RichOptionList::~RichOptionList()
{
